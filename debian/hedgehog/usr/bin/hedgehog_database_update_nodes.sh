...
# 
# File:   hedgehog_database_update_nodes.sh
#
...
while read line || [[ -n "$line" ]]; do
	...
	# Now add the data
	SERVER_ID=`psql $DB_NAME -tc "select id from server where display_name=$SQ$SERVER_NAME$SQ"`
	...
	if [ -z "$SERVER_ID" ] ; then
		...
		if [ ${PREDICT} -eq 0 ] ; then
			...
			psql -d $DB_NAME -c "INSERT INTO dsc.server (name, display_name) VALUES ('$SERVER_DB_NAME', '$SERVER_NAME');"
			SERVER_ID=`psql $DB_NAME -tc "select id from server where display_name=$SQ$SERVER_NAME$SQ"`
	...
	NODE_ID=`psql $DB_NAME -tc "select id from dsc.node where server_id=$SERVER_ID and name=$SQ$NODE_NAME$SQ"`
	
	if [ -z "${NODE_ID}" ] ; then 
		echo "INFO: New node found - inserting node with name name: '$NODE_NAME' on server: '$SERVER_NAME' with group: '$GROUP'"
		if [ ${PREDICT} -eq 0 ] ; then
			psql -d $DB_NAME -c "INSERT INTO dsc.node (server_id, name, region) VALUES ($SERVER_ID, '$NODE_NAME', '$GROUP');"
			mkdir -p $DATA_DIR/$SERVER_NAME/$NODE_NAME/incoming
		fi		
	else
		DB_RESULT=`echo "SELECT region FROM dsc.node where server_id=$SERVER_ID and id=$NODE_ID;" | psql -td $DB_NAME  | sed -e 's/^ *//' -e 's/ *$//'`
		if [ "${GROUP}" != "${DB_RESULT}" ] ; then
			...
			if [ ${PREDICT} -eq 0 ] ; then
				psql -d $DB_NAME -c "UPDATE dsc.node SET region = '$GROUP' WHERE id=$NODE_ID;"	
			fi
		fi
	fi
done < ${NODES_FILE}
...
if [ ${NEW_SERVER} -eq 1 ] && [ $SUPPRESS_WARNINGS -eq 0 ] ; then
	...
	echo "server the 'hedgehog_manage_partitions.sh' script MUST be run manually now."
	echo "********************************************************************************"
	echo
fi
