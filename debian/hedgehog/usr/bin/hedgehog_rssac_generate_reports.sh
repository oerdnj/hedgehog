...
usage () {
	...
	echo "  -D Also delete the raw unique_source data (default: no delete)"
...
if [ ${PROCESS_UNIQUE_SOURCES} -eq 1 ] ; then
	...
	echo "/usr/bin/hedgehog_process_unique_source_data.sh -s ${START} ${DELETE_RAW_DATA_FLAG}"
	/usr/bin/hedgehog_rssac_process_unique_source_data.sh -s ${START} ${DELETE_RAW_DATA_FLAG}
	echo
fi

PLOTIDS=`echo "SELECT id FROM dsc.plot where ddcategory='9.RSSAC' and name != '$TRAFFIC_VOLUME_DIFF' ORDER BY id DESC;" | psql -td $DB_NAME`
...
for SVRNM in $SERVERNMS ; do
    ...
    for TYPE in $TYPES ; do
        ...
        for PLTID in $PLOTIDS ; do
			PLTNM=`echo "SELECT name FROM dsc.plot where id=${PLTID};" | psql -td $DB_NAME | sed 's/[[:space:]]//g'`
            ...
            if [ "${TYPE}" == "generatePlot" ] ; then
                ...
                if [ "${PLTFILE}" != "plots/no_graph.png" ] && [ "${PLTFILE}" != "plots/no_results.png" ] && [ "${PLTFILE}" != "plots/no_connection.png" ] ; then                   
					...
					REP_NAME=`echo "SELECT name FROM dsc.plot where id=${PLTID};" | psql -td $DB_NAME | sed 's/_/-/g' | sed 's/[[:space:]]//g'`
