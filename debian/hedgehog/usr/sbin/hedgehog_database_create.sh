...
sudo -u $hhconfig_database_owner psql -d $hhconfig_database_name < /usr/share/hedgehog/sql/initialise_database.sql
echo
...
sudo -u $hhconfig_database_owner psql -d $hhconfig_database_name  < /usr/share/hedgehog/sql/insert_lookup_data.sql #> /dev/null

echo "Populate servers and nodes"
sudo -u $hhconfig_database_owner /usr/bin/hedgehog_database_update_nodes.sh -s
###########################################################
...
sudo -u $hhconfig_database_owner /usr/bin/DDL_updates/000009_ddl_traffic_difference.sh
###########################################################
